...
static void convert(void){for(int i=0;i<WIDTH*HEIGHT;i++){rgb24[i*3]=(uint8_t)red(demo_palette[pixels[i]]);rgb24[i*3+1]=(uint8_t)green(demo_palette[pixels[i]]);rgb24[i*3+2]=(uint8_t)blue(demo_palette[pixels[i]]);}}
...
int main(int argc,char**argv){
    int vhash=0,raw=0,headless=0,mute=0,frames=-1,fps=60;float start=0;const char*shot=NULL,*wav=NULL;
    for(int i=1;i<argc;i++){
        if(!strcmp(argv[i],"--vhash")){vhash=1;headless=1;}
        else if(!strcmp(argv[i],"--raw")){raw=1;headless=1;}
        ...
        else{fprintf(stderr,"DARKROOM: --start seconds --shot file.ppm --wav file.wav --raw --frames N --fps N --headless --mute\n");return 2;}
    }
    ...
    if(vhash){for(unsigned t=0;t<=80;t+=10){demo_render(pixels,t*SAMPLE_RATE);printf("VHASH s=%u %08x\n",t*SAMPLE_RATE,demo_frame_hash(pixels));}return 0;}
    ...
    if(!headless){
        ...
        w=SDL_CreateWindow("Darkroom / Stellar (1994) - RP2350 recreation by Phase",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,960,720,SDL_WINDOW_RESIZABLE|SDL_WINDOW_ALLOW_HIGHDPI);
    ...
    while(rendered<frames&&!quit){
        ...
        if(headless&&sample>=DURATION_SAMPLES)break;
    ...
    fprintf(stderr,"DARKROOM: %d frames, render mean %.2f ms, worst %.2f ms, max %u triangles (HOST measurements)\n",rendered,rendered?(double)total_us/rendered/1000:0,(double)max_us/1000,max_tri);
