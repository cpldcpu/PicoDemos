...
/* The PREVIOUS frame, read-only. HYSTERESIS needs this and SUSTAIN did not:
 * a feedback system computes frame n from frame n-1, and MODE_320's existing
 * double buffer already holds exactly that. Safe to read while it is being
 * scanned out -- the scanline callback is a reader too. */
const uint8_t *vga_320_front_buffer(void);
