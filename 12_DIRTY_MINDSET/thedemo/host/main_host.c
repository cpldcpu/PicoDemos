 ...
 *   --dump-frames S D  : dump sequential frames starting at S ms for D ms, then exit
 *
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "--start-ms") && i + 1 < argc)
            ...
            offline_mode = 1;
        else if (!strcmp(argv[i], "--dump-frames") && i + 2 < argc) {
            dump_start_ms = atoi(argv[++i]);
            dump_duration_ms = atoi(argv[++i]);
        }
        else {
    ...
    if (dump_start_ms>=0)  printf("  --dump-frames %d %d\n", dump_start_ms, dump_duration_ms);
    ...
    while (!vga_should_quit()) {
        ...
        /* Take screenshot for sequential frame dumping */
        if (!offline_mode) {
            if (dump_start_ms >= 0 && (int)t >= dump_start_ms && (int)t < dump_start_ms + dump_duration_ms) {
                vga_screenshot();
            }
            if (dump_start_ms >= 0 && (int)t >= dump_start_ms + dump_duration_ms) {
                break; /* Done dumping frames! */
            }
