...
int main(int argc,char**argv){
    ...
    for(int i=1;i<argc;i++){
        ...
        else{fprintf(stderr,"TESSERA: --start seconds --shot file.ppm --wav file.wav --raw --frames N --fps N --headless --mute\n");return 2;}
    }
    ...
    if(vhash){for(unsigned t=0;t<=150;t+=30){demo_render(pixels,t*SAMPLE_RATE);printf("VHASH s=%u %08x\n",t*SAMPLE_RATE,demo_frame_hash(pixels));}return 0;}
    ...
    if(!headless){
        ...
        w=SDL_CreateWindow("TESSERA / LATENT / 2026",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,960,720,SDL_WINDOW_RESIZABLE|SDL_WINDOW_ALLOW_HIGHDPI);
    ...
    fprintf(stderr,"TESSERA: %d frames, render mean %.2f ms, worst %.2f ms, max %u triangles (HOST measurements)\n",rendered,rendered?(double)total_us/rendered/1000:0,(double)max_us/1000,max_tri);
