...
int main(void){
    ...
    for(unsigned sample=0;sample<=DURATION_SAMPLES;sample+=400){
        ...
        if(sample%(SAMPLE_RATE/2)==0){
            uint64_t same=hash_frame();memset(guarded.pixels,0x3c,sizeof guarded.pixels);demo_render(guarded.pixels,sample);
            if(same!=hash_frame())fail("incomplete framebuffer write",sample);
        }
    ...
    for(unsigned pos=0;pos<DURATION_SAMPLES;pos+=300){
        synth_render(block,300);for(unsigned i=0;i<600;i++)expected=(expected^(uint16_t)audio[pos*2+i])*16777619u;
    ...
    if(synth_final_hash()!=expected)fail("final audio hash",DURATION_SAMPLES);
    free(audio);
    printf("PASS 9217 guarded frames; deterministic seek; audio arbitrary blocks and endpoint\n");
    printf("visual_hash=%016llx audio_hash=%016llx max_triangles=%u audio_peak=%u (Phase score)\n",(unsigned long long)combined,(unsigned long long)ah,maxtri,peak);
    return 0;
}
