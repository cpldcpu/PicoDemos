...
ASSETS = [
    ...
    # title_bg packed at 320x240 for MODE_320.
    # reserved=32 leaves slots 224..255 free for the copperbar gradient.
    ("title_bg",        "title_bg_640.png",        "8bpp",   320, 240, True,   None,                                       32),
    # voxel_color packed with reserved=224 — only 32 base colours so we can
    # build a 7-band depth-fog gradient at runtime (32 × 7 = 224 slots).
    # The renderer composes the final palette index as (shade<<5)|base.
    ("voxel_color",     "voxel_color_1024.png",    "8bpp",   512, 512, True,   None,                                       224),
    # Heightmap is *derived* from the colormap (luminance → blur → level
     # stretch). The supplied voxel_height_1024.png turned out to be
     # high-frequency noise rather than a coherent landscape, so we
     # synthesise it from voxel_color where the ridge/valley structure
     # already exists. See derive_heightmap() for the recipe.
    ("voxel_height",    None,                      "gray8",  512, 512, False,  "heightmap_from:voxel_color_1024.png"),
    # Envmap packed as 8bpp so MODE_320 (chunky palette) can render
    # the spheres scene. reserved=32 leaves slots 224..255 for sphere
    # tinted-shading ramps (4 spheres × 8 shade levels each).
    ("envmap",          "envmap_equi_2048.png",    "8bpp",   256, 128, True,   None,                                       32),
    ("portrait_a",      "portrait_a_512.png",      "8bpp",    96,  96, True,   None),
    ("portrait_b",      "portrait_b_512.png",      "8bpp",    96,  96, True,   None),
    ("portrait_c",      "portrait_c_512.png",      "8bpp",    96,  96, True,   None),
    ("logo_mask",       "logo_mask_512.png",       "1bpp",   160, 120, False,  None),
    # Tunnel packed with reserved=224 — only 32 base colours so we can
    # build a full 8-shade depth gradient at runtime (32 × 8 = 256 slots).
    # The renderer composes the final palette index as (shade<<5)|base,
    # giving smooth depth shading instead of stippled dithering.
    ("tunnel_tex",      "tunnel_tex_512.png",      "8bpp",   256, 256, True,   None,                                       224),
    # Packed at 320×240 so MODE_320 can blit it directly. reserved=32
    # leaves slots 224..255 free for scroller text + ambient accents.
    ("greetz_bg",       "greetz_bg_640.png",       "8bpp",   320, 240, True,   None,                                       32),
    # Scene 7 — particle target shape, 1bpp.
    ("endcard_mask",    "endcard_mask_512.png",    "1bpp",   300,  80, False,  None),
    # Scene 8 — outro backdrop, packed at 320x240 like title for now.
    ("endcard_bg",      "endcard_640.png",         "8bpp",   320, 240, True,   None,                                       32),
]
