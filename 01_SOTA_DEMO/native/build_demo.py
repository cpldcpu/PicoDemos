...
# 'mod' = pack original MOD/MP3/WAV files (used by desktop mikmod/SDL_mixer).
# 'qoa' = transcode all audio to mono QOA at QOA_RATE and pack the QOA bytes
# instead (used by the Pico VGA build, which has no MOD synth and limited
# flash). Set by --audio-format on the command line.
AUDIO_FORMAT = 'mod'
QOA_RATE = 11025

def _audio_as_qoa(filename):
	"""Transcode an audio file (MOD / WAV / MP3 / anything ffmpeg-libopenmpt
	can read) to mono QOA at QOA_RATE and return the .qoa filename. Caches
	to data/qoa_<rate>/ so repeated builds are fast."""
	if not filename.lower().endswith(('.mod', '.wav', '.mp3')):
		return filename
	cache_dir = os.path.join('data', 'qoa_%d' % QOA_RATE)
	os.makedirs(cache_dir, exist_ok=True)
	stem = os.path.splitext(os.path.basename(filename))[0]
	qoa_path = os.path.join(cache_dir, stem + '.qoa')
	if os.path.exists(qoa_path) and os.path.getmtime(qoa_path) >= os.path.getmtime(filename):
		return qoa_path
	tmp_wav = os.path.join(cache_dir, stem + '.wav')
	# 1) decode to canonical mono 16-bit PCM at QOA_RATE via ffmpeg
	subprocess.run(['ffmpeg', '-y', '-hide_banner', '-loglevel', 'error',
		'-i', filename, '-ac', '1', '-ar', str(QOA_RATE),
		'-sample_fmt', 's16', tmp_wav], check=True)
	# 2) encode that WAV to QOA via the host qoaconv tool (sota/pico/qoaconv.exe)
	qoaconv = os.path.join(os.path.dirname(__file__), '..', 'pico', 'qoaconv.exe')
	if not os.path.exists(qoaconv):
		qoaconv = os.path.join(os.path.dirname(__file__), '..', 'pico', 'qoaconv')
	subprocess.run([qoaconv, tmp_wav, qoa_path], check=True)
	os.remove(tmp_wav)
	print("  -> %s (%d bytes)" % (qoa_path, os.path.getsize(qoa_path)))
	return qoa_path

def audio_file(filename):
	"""Return the actual filename to pack into the WAD for the given audio
	asset, honoring --audio-format."""
	if AUDIO_FORMAT == 'qoa':
		return _audio_as_qoa(filename)
	return filename
...
def encode_music(args, state):
	if args['type'] == 'start':
		if USE_MODS:
			get_file(args['mod'])
			packme = (CMD_MOD, 1, state['wad'].add(audio_file(args['mod'])))
		else:
			get_file(args['mp3'])
			packme = (CMD_MP3, 1, state['wad'].add(audio_file(args['mp3'])))
	elif args['type'] == 'stop':
		if USE_MODS:
			packme = (CMD_MOD, 2, 0)
		else:
...
def encode_sound(args, state):
	get_file(args['name'])
	file_idx = state['wad'].add(audio_file(args['name']))
...
if __name__ == '__main__':
	...
	parser.add_argument('--audio-format', default='mod', choices=['mod', 'qoa'],
		help='How audio assets are stored in the WAD. "mod" = original tracker/WAV/MP3 files for desktop+mikmod. "qoa" = transcode to 11 kHz mono QOA for the Pico VGA build.')
	parser.add_argument('--qoa-rate', type=int, default=11025, help='Sample rate (Hz) for QOA audio. 11025 fits SOTA in the Pico\'s ~1.1 MB free flash; higher rates may not fit.')
	parser.add_argument('--output', default='sota.wad', help='Output WAD filename.')
	args = parser.parse_args()

	USE_MODS = not args.mp3
	AUDIO_FORMAT = args.audio_format
	QOA_RATE = args.qoa_rate

	build_demo(DEMO, args.output)
