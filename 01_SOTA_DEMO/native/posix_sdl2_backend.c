...
{
	/* Drain every queued event each frame so the OS message pump stays
	 * responsive (otherwise Windows marks the window 'Not Responding' and
	 * it becomes undraggable). */
	SDL_Event event;
	...
	if(time_remaining_this_frame > 0) {
		SDL_Delay((Uint32)time_remaining_this_frame);
	}
