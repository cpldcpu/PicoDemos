...
void graphics_draw_scaled_polygon_to_bitmap(int num_vertices, uint8_t *data, float scalex, float scaley, int xofs, int yofs, struct Bitplane *bitplane)
{
	...
	/* A single-vertex polygon has nothing to outline, and would leave x1
	 * and y1 uninitialized when the close-shape line is drawn below.
	 * The Bresenham in planar_line_thick has no iteration cap, so on a
	 * platform where the stack happens to hold large garbage (e.g.
	 * Cortex-M0+), this can look like a hang. */
	if(num_vertices < 2)
		return;
