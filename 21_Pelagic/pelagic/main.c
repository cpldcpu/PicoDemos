/* PELAGIC on the board. Phase's loop, with the telemetry the score has to be
 ...
 */
void __attribute__((noreturn)) pelagic_panic(const char *fmt, ...)
{
    puts("\n*** PANIC ***");
    if (fmt) {
        va_list ap;
...
{
    return bar < 8 ? 0 : bar < 24 ? 1 : bar < 40 ? 2 : bar < 48 ? 3 : bar < 64 ? 4 : bar < 72 ? 5 : 6;
}

int main(void){
    vreg_set_voltage(VREG_VOLTAGE_1_20);sleep_ms(10);set_sys_clock_khz(300000,true);
    stdio_init_all();
    printf("\nBOOT PELAGIC smooth=%d interp=%d sys=%lu Hz\n",
           PELAGIC_SMOOTH,PELAGIC_INTERP,(unsigned long)clock_get_hz(clk_sys));
    demo_init();audio_init();video_init();audio_start();
    ...
    while(audio_position()<DURATION_SAMPLES){
        ...
        if(now-report>=1000000){
            ...
            printf("T t=%lu.%lu bar=%lu sec=%d smooth=%d interp=%d"
                ...
                (unsigned long)bar,section_of(bar),PELAGIC_SMOOTH,PELAGIC_INTERP,
