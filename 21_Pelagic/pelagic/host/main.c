...
int main(int argc,char**argv){
    ...
    for(int i=1;i<argc;i++){
        ...
        else{fprintf(stderr,"PELAGIC: --start seconds --shot file.ppm --wav file.wav --raw --frames N --fps N --headless --mute\n");return 2;}
    }
    ...
    if(!headless){
        ...
        w=SDL_CreateWindow("PELAGIC / LATENT / 2026",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,960,720,SDL_WINDOW_RESIZABLE|SDL_WINDOW_ALLOW_HIGHDPI);
    ...
    fprintf(stderr,"PELAGIC: %d frames, render mean %.2f ms, worst %.2f ms, max %u triangles (HOST measurements)\n",rendered,rendered?(double)total_us/rendered/1000:0,(double)max_us/1000,max_tri);
