/* Host harness for the PELAGIC score: renders synth.c + song.c to a WAV with
 * a chosen block size and solo mask, or dumps the note tables as text for the
 * piano roll. Built by song_check.py / song_roll.py; not part of either target.
 *
...
static void write_wav(const char *path, int chunk, unsigned solo)
{
    ...
    uint32_t n = DURATION_SAMPLES, data = n * 4, riff = data + 36, rate = SAMPLE_RATE, br = rate * 4, fmt = 16;
...
static void hashes(void)
{
    ...
    for (uint32_t p = 0; p < DURATION_SAMPLES;) {
        ...
        p += k;
        uint32_t hp = 0, hh = 0;
        if (synth_hash_latch(&hp, &hh) && hp != last) { printf("H %u %08x\n", (unsigned)hp, (unsigned)hh); last = hp; }
    }
}

static void dump(void)
{
    printf("# step bass pluck lead lead2 drums\n");
    for (uint32_t s = 0; s < SONG_BARS * SONG_STEPS_PER_BAR; s++)
        printf("S %u %d %d %d %d %u\n", (unsigned)s, song_bass(s), song_pluck(s),
               song_lead(s), song_lead2(s), (unsigned)song_drums(s));
    printf("# bar section transpose pad0 pad1 pad2 pad3 lead_lvl lead_cut lead2_lvl pad_lvl shimmer energy voices glass choir pluck_lvl drone\n");
    for (uint32_t b = 0; b < SONG_BARS; b++) {
        uint8_t c[4]; song_pad_chord(b, c);
        printf("B %u %d %d %u %u %u %u %d %d %d %d %d %d %u %d %d %d %d\n", (unsigned)b, song_section(b),
               song_transpose(b), c[0], c[1], c[2], c[3], song_lead_level(b), song_lead_cut(b),
               song_lead2_level(b), song_pad_level(b), song_shimmer(b), song_energy(b),
               (unsigned)song_voices(b), song_glass_level(b), song_choir_level(b),
               song_pluck_level(b), song_drone_level(b));
    }
}
