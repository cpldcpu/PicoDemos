/* accelerator.h -- power-of-two indexed texture addressing and palette
 * blending through the RP2350's SIO interpolators. Lifted from HELION with
 * the guards renamed and the self-test kept verbatim; HELION's
 * background_begin()/background_wait() went with it, generalised to take the
 * source, because SLEEPER's sky is generated until Phase's plates land and
 * the DMA still wants to run under the layer work.
 *
 * INTERP1 POP returns a byte offset and advances U and V. INTERP0 BLEND
 * builds a palette outside the pixel loop. No colour arithmetic in the
 * texture loop.
 */
#ifndef SLEEPER_ACCELERATOR_H
#define SLEEPER_ACCELERATOR_H
#include <stdint.h>
#include <string.h>
#ifndef SLEEPER_INTERP
#define SLEEPER_INTERP 1
#endif
#ifndef SLEEPER_DMA
#define SLEEPER_DMA 1
#endif
...
#if !defined(PICO_BUILD) || !SLEEPER_INTERP
...
#endif
#if defined(PICO_BUILD) && SLEEPER_DMA
    dma_channel_claim(8);
#endif
}
...
static inline void background_begin(uint16_t *dst, const uint16_t *src, unsigned pixels)
{
    ...
    dma_channel_configure(8, &c, dst, src, pixels / 2, true);
#else
