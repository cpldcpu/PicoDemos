#!/usr/bin/env python3
"""Read SLEEPER's USB CDC telemetry, and be referees 2 and 4 while doing it.

COLOSSUS's tool, with the two counters PLANNING §2's first claim needs:
`repeat N`, the fields at which scanline zero found no pending page and
the previous picture was shown again, and `over N`, the frames whose
demo_render() took longer than one VGA field. Both are cumulative and
both are on the DONE line, so a run judged from a saved log gives the
same verdict as one judged live. The claim is that `repeat` is zero over
the whole 192 s with the score running. -- Overscan
...
def main():
    ...
    stats = {"lines": 0, "checked": 0, "bad": 0, "unknown": 0,
             ...
             "under": 0, "repeat": 0, "over": 0, "fps_min": None, "copy": [], "synth": []}
    seen, bad = set(), []
    ...
    for text in source:
        ...
        for key, rx in NUM_RE.items():
            ...
            elif key in ("miss", "late", "under", "repeat", "over"):
    ...
    print(f"  repeated fields   {stats['repeat']}"
          f"   (PLANNING 2.1: must be 0 on the shipping UF2)")
    print(f"  frames over 16 ms {stats['over']}")
