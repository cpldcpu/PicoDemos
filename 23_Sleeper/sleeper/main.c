/* SLEEPER on the board. HELION's loop and HELION's telemetry line, with the
 * two counters PLANNING §2's first claim needs:
 *
 *   repeat N   fields at which scanline zero found no pending page, i.e.
 *              vsyncs that showed the previous picture again (video.c).
 *   over N     frames whose demo_render() took more than 16,000 us, so more
 *              than one VGA field. A frame can be `over` without being a
 *              `repeat` only at the very edge of the field; the two together
 *              say whether a shot is late and by how much.
 *
 * Both are cumulative and both appear on the DONE line. Every other token is
 * spelled exactly as HELION spelled it so the COLOSSUS serial tools parse
 * this unchanged. -- Overscan
 */
#include "device.h"
#include "song.h"
 ...
 */
void __attribute__((noreturn)) sleeper_panic(const char *fmt, ...)
{
    puts("\n*** PANIC ***");
    if (fmt) {
        va_list ap;
...
int main(void)
{
     ...
     * enumeration. Wait for CDC, but bounded: a board on bare power is
     * delayed 2 s at most and then plays exactly as before. */
    for (int i = 0; i < 200 && !stdio_usb_connected(); i++) sleep_ms(10);
    printf("\nBOOT SLEEPER dma=%d interp=%d sys=%lu Hz\n",
           SLEEPER_DMA, SLEEPER_INTERP, (unsigned long)clock_get_hz(clk_sys));
    song_init();
    demo_init();
    ...
    printf("SELFTEST accelerator=%d (%s)\n", accelerator_selftest(),
           SLEEPER_INTERP ? "real SIO registers" : "software model");
    audio_init(); video_init();
    /* Prime the pipeline before the clock starts. The first call into
     * demo_render() is the expensive one -- it builds the first sky
     * palette, first-touches both pages and takes every XIP miss the
     * renderer has -- and if it happens after audio_start() it costs one
     * field, which is one repeat against a claim that is binary. Drawing
     * sample 0 and presenting it here puts a picture on the wire before
     * the DAC begins, and the loop's first frame gets a whole field to
     * itself. Twice, because there are two pages and the first write
     * to each is the one that misses: priming once leaves the second
     * page cold and the loop's first frame still costs a field.
     * -- Overscan */
    ...
    while (audio_position() < DURATION_SAMPLES) {
        ...
        if (now - report >= 1000000) {
            ...
            /* Token spellings follow COLOSSUS's and HELION's telemetry line so
            ...
            printf("T t=%lu.%lu bar=%lu sec=%d dma=%d interp=%d"
                ...
                (unsigned long)bar, song_section(bar), SLEEPER_DMA, SLEEPER_INTERP,
            ...
            printf(" | last_us prep=%lu world=%lu lights=%lu board=%lu other=%lu"
                   " shot=%u world_id=%u lights_n=%lu",
                (unsigned long)st.prepare, (unsigned long)st.world, (unsigned long)st.lights_us,
                (unsigned long)st.board_us, (unsigned long)st.other,
                st.shot, st.world_id, (unsigned long)st.lights);
            printf("\n");
