# 11_FlashDemo (VOLTAGE)

An original demoscene production for the **Raspberry Pi Pico 2 (RP2350)**, targeting Waveshare RP2350-Plus + Pimoroni VGA Demo Base. Premium electric cyber-neon aesthetic, three VGA modes plus a copper-style raster split, QOA-encoded soundtrack, AI-generated still imagery.
...
📺 **[Download & Play: VOLTAGE 60FPS HD Demo Video (media/voltage_demo_60fps.mp4)](media/voltage_demo_60fps.mp4)**

Direct video file:

- [media/voltage_demo_60fps.mp4](media/voltage_demo_60fps.mp4)

## Prebuilt Firmware

The checked-in RP2350 VGA firmware is [voltage_vga_rp2350.uf2](voltage_vga_rp2350.uf2).
Hold BOOTSEL while plugging in the Pico 2, then drag the UF2 onto the RPI-RP2
USB drive.

### Scene Gallery
| Spark-Gap Grid (Intro) | Unstable Fluid Reactor | Ray-Volt Chorus | Spark Generator |
|:---:|:---:|:---:|:---:|
| ![Spark-Gap Grid](media/spark_gap.png) | ![Plasma Core](media/plasma_core.png) | ![Ray-Volt](media/ray_volt.png) | ![Spark Generator](media/spark_generator.png) |

| Vector Strike | Canyon Flight (Climax) | Julia Shockwave | Voltage Arc (Outro) |
|:---:|:---:|:---:|:---:|
| ![Vector Strike](media/vector_strike.png) | ![Canyon Flight](media/canyon_flight.png) | ![Julia Shockwave](media/julia_shockwave.png) | ![Voltage Arc](media/voltage_arc.png) |
...
is [voltage_vga_rp2350.uf2](voltage_vga_rp2350.uf2).
...
The MP3 source lives at [thedemo/assets/VOLTAGE CONNECTING.mp3](thedemo/assets/VOLTAGE%20CONNECTING.mp3); the build incbins the encoded `thedemo/music.qoa` next to the rest of the SDK sources. To re-encode after swapping the MP3:

1. Convert to mono 22050 Hz s16le PCM with ffmpeg:
   ```bash
   ffmpeg -i "thedemo/assets/VOLTAGE CONNECTING.mp3" -ac 1 -ar 22050 -f s16le music.raw
   ```
...
```bash
python thedemo/tools/analyze_music.py "thedemo/assets/VOLTAGE CONNECTING.mp3" -k 8
```
...
- **Code, Optimization & Direction**: Antigravity (Google DeepMind)
- **Base Engine Framework**: Adapted from *TheDemo (Project 20)* by Claude Opus 4.7 (Anthropic)
- **Human Nudging**: Azure
- **Music**: Generated with Suno 4.5, prompt engineering by Antigravity
- **Graphics**: Nano Banana Pro (Google) with prompts & post-processing recipes by Antigravity
- **QOA Codec**: Dominic Szablewski (MIT) — `thedemo/qoa.h`
- **Pico SDK & Libraries**: Raspberry Pi / Raspberry Pi Pico SDK & pico-extras
