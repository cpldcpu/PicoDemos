...
/* MODE_HIRES — 320x240 RGB565 truecolor, 1:1 (no doubling). */
static uint16_t fb_hires[VGA_HIRES_W * VGA_HIRES_H];
...
static void take_screenshot(void)
{
    ...
    for (int y = 0; y < VGA_320_H; y++) {
        if (g_mode == MODE_HIRES) {
            for (int x = 0; x < VGA_320_W; x++) {
                uint16_t c = fb_hires[y * VGA_HIRES_W + x];
...
/* Convert one PIO-native RGB565 pixel (see rgb565.h: B in high bits, R in
 * low bits — NOT standard RGB565) to ARGB8888. Feeding the raw value to an
 * SDL RGB565 texture would swap R/B; we must convert explicitly so the host
 * preview matches the device and the screenshots. */
...
{
    /* Convert + 2× pixel/line-double into the ARGB8888 tex (correct colours). */
    ...
    for (int y = 0; y < VGA_320_H; y++) {
        const uint16_t *src = &fb160[(y >> 1) * VGA_160_W];
        uint32_t *dst = out + y * stride;
        for (int x = 0; x < VGA_320_W; x++) dst[x] = pio565_to_argb(src[x >> 1]);
    }
    SDL_UnlockTexture(tex);
...
/* --- MODE_HIRES — 320x240 RGB565, 1:1 --------------------------------- */

uint16_t *vga_hires_back_buffer(void) { return fb_hires; }

void vga_hires_present(void)
{
    /* Convert PIO-native RGB565 → ARGB8888 into the main tex (correct
     * colours; a raw RGB565 texture would swap R/B). */
    void *pixels = NULL; int pitch = 0;
    if (SDL_LockTexture(tex, NULL, &pixels, &pitch) != 0) {
        fprintf(stderr, "SDL_LockTexture(hires): %s\n", SDL_GetError());
        return;
    }
    uint32_t *out = (uint32_t *)pixels; int stride = pitch / 4;
    for (int y = 0; y < VGA_HIRES_H; y++) {
        const uint16_t *src = &fb_hires[y * VGA_HIRES_W];
        uint32_t *dst = out + y * stride;
        for (int x = 0; x < VGA_HIRES_W; x++) dst[x] = pio565_to_argb(src[x]);
    }
    SDL_UnlockTexture(tex);
