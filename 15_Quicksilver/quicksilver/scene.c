...
static uint32_t        g_scene_end_ms   = 0;
...
int scene_runner_tick(uint32_t t_ms_global)
{
    ...
    if (idx != g_active_i) {
        ...
        g_scene_end_ms   = timeline[idx].end_ms;
    ...
    if (g_active && g_active->frame) {
        ...
        /* uniform liquid-chrome glint at every scene boundary (all scenes are
         * MODE_HIRES). Kept in the runner so no scene needs to wire it. The
         * final scene suppresses its out-glint and fades to black itself. */
        if (g_active->mode == MODE_HIRES) {
            int last = (idx == timeline_count - 1);
            int out_style = timeline_trans[idx];
            int in_style  = timeline_trans[idx > 0 ? idx - 1 : 0];
            qs_transition_apply(t_ms_global, g_scene_start_ms, g_scene_end_ms,
                                last, out_style, in_style);
        }
    }
...
uint32_t scene_cur_start_ms(void) { return g_scene_start_ms; }
uint32_t scene_cur_end_ms(void)   { return g_scene_end_ms; }
