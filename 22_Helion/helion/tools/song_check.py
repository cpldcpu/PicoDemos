#!/usr/bin/env python3
"""Level and determinism checks for the HELION score.
...
Builds tools/song_harness.c against ../synth.c and ../song.c, then:
  ...
  1. renders the whole piece with block sizes 1, 8 and 1024 and asserts the
     three WAVs are byte-identical (the pull-model contract in helion.h);
...
RATE, BARS, BAR_SAMPLES = 24000, 80, 48000
SOLO = {"drum": 1, "rim": 2, "scrape": 4, "bass": 8, "bronze": 16, "reed": 32, "bow": 64,
        "fx": 128, "harm": 256, "gong": 512, "air": 1024}
SV = {"drum": 1, "rim": 2, "scrape": 4, "bass": 8, "bronze": 16, "reed": 32, "bow": 64,
      "harm": 128, "air": 256, "gong": 512}
EVENT_COL = {"bass": 1, "bronze": 2, "reed": 3, "harm": 4, "bow": 6}
...
def render(name, chunk=1024, solo=2047):
...
def main():
    ...
    for b in range(BARS):
        ...
        for n, bit in SV.items():
            ...
            if solo[n][1][b] < (4 if n == "air" else 15):      # the air is meant to be barely there
                problems.append("bar %d: %s is arranged but silent (rms %.1f)" % (b, n, solo[n][1][b]))
