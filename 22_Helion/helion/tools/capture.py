...
def main():
    ...
    parser.add_argument('--exe',type=Path,default=Path(__file__).resolve().parents[1]/'build_host'/'helion.exe')
    parser.add_argument('--out',type=Path,default=Path(__file__).resolve().parents[2]/'media'/'helion.mp4')
    parser.add_argument('--fps',type=int,default=30)
    args=parser.parse_args()
    args.out.parent.mkdir(parents=True,exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='helion-') as tmp:
