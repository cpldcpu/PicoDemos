...
#ifndef HELION_SILENT_PUMP
#define HELION_SILENT_PUMP 0
#endif
#define RING 1024u
...
/* HELION_SILENT_PUMP (off by default, never in a release build) replaces the
 * synth with silence while keeping the ring, the DMA polling and the same
 * eight-frame chunking, so a run with it prices what audio_pump() costs core
 * 1 with no DSP in it. Subtracting that from a normal run is the only way to
 * price the synth itself without editing synth.c, which is not mine.
 * -- Overscan */
#if HELION_SILENT_PUMP
...
static void fill(unsigned n){
    int16_t samples[16];
#if HELION_SILENT_PUMP
    while(n){unsigned k=n>8?8:n;for(unsigned i=0;i<k*2;i++)samples[i]=0;
...
void HOT(audio_pump)(void){
     ...
     * whatever is still in the ring, which is an audible underrun. Count them. */
