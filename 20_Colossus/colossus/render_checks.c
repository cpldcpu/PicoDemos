...
int main(void)
{
    ...
    /* Final composition: the accepted body stays 160–180 native pixels tall.
       Render just geometry to measure silhouette, excluding sky/text/particles. */
    r_begin(guarded.pixels,8,scene_camera(136*CV_BAR,8));r_background(136*CV_BAR);
    memset(guarded.pixels,0,sizeof guarded.pixels);body_pose(136*CV_BAR);body_draw(-1,0);
    int top=240,bottom=-1;
    for(int y=0;y<240;y++)for(int x=0;x<320;x++)if(guarded.pixels[y*320+x]){if(y<top)top=y;if(y>bottom)bottom=y;}
