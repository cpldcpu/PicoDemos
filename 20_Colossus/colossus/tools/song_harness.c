/* Host harness for the COLOSSUS song: renders synth.c + song.c to a WAV with a chosen
...
static void write_wav(const char *path, int chunk, unsigned solo)
{
    ...
    uint32_t n = CV_TOTAL_SAMPLES, data = n * 4, riff = data + 36, rate = CV_RATE, br = rate * 4, fmt = 16;
...
static void dump(void)
{
    ...
    for (uint32_t s = 0; s < (uint32_t)CV_BARS * CV_STEPS_PER_BAR; s++)
